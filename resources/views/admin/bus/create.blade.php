...
<head>
  <meta charset="UTF-8">
  <title>KlikBus - Tambah Armada</title>
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .kb-content { padding: 40px; flex: 1; display: flex; justify-content: center; align-items: flex-start; }
    
    .kb-form-card { background: #fff; width: 100%; max-width: 600px; padding: 40px; border-radius: 20px; border: 1px solid var(--color-border); box-shadow: 0 10px 25px rgba(0,0,0,0.02); }
...
<div class="kb-shell">
  ...
  <main class="kb-main">
    <div class="kb-topbar">
      <div style="font-family: 'Sora', sans-serif; font-size: 22px; font-weight: 600;">TAMBAH ARMADA BARU</div>
    </div>

    <div class="kb-content">
      <div class="kb-form-card">
        <div class="kb-form-title">Informasi Armada</div>
        <p style="color: #9ca3af; margin-bottom: 30px;">Input data bus baru untuk operasional KlikBus Lampung.</p>
        
        <form action="{{ route('admin.bus.store') }}" method="POST">
          @csrf
          <div class="form-group">
            <label>Nama Armada Bus</label>
            <input type="text" name="nama_armada" placeholder="Contoh: KlikBus Trans Lampung 01" required>
          </div>

          <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div>
              <label>Kapasitas (Kursi)</label>
              <input type="number" name="kapasitas" placeholder="40" required>
            </div>
            <div>
          ...
          <button type="submit" class="btn-save">Simpan Armada Baru</button>
