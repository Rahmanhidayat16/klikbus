...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* MAIN */
    .kb-main { flex: 1; display: flex; flex-direction: column; height: 100vh; overflow: hidden; }
    .kb-topbar {
    ...
    .kb-content { padding: 25px 40px; flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 20px; }
...
<div class="kb-shell">
  ...
  <aside class="kb-sidebar">
    ...
    <nav class="kb-nav">
      ...
      <a href="{{ route('admin.dashboard') }}" class="kb-nav-item">
        <i class="ti ti-layout-dashboard"></i> Dashboard
      </a>
      <a href="{{ route('admin.bus.index') }}" class="kb-nav-item">
        <i class="ti ti-bus"></i> Kelola Bus
        <span class="kb-badge">{{ \App\Models\Bus::count() }}</span>
      </a>
      <a href="{{ route('admin.routes.index') }}" class="kb-nav-item active">
        <i class="ti ti-route"></i> Atur Rute
        <span class="kb-badge">{{ $routes->count() }}</span>
      </a>
      <a href="{{ route('admin.schedules.index') }}" class="kb-nav-item">
        <i class="ti ti-calendar-event"></i> Jadwal
        <span class="kb-badge">{{ $total_jadwal }}</span>
      </a>

      <div class="kb-nav-label">Laporan & Keuangan</div>
      <a href="{{ route('admin.reports.bookings') }}" class="kb-nav-item">
    ...
    <div class="kb-sidebar-footer">
      ...
      <form method="POST" action="{{ route('logout') }}" style="display:inline;">
        @csrf
        <button type="submit" style="background:none; border:none; cursor:pointer; color:#ef4444; font-size:20px; padding:8px; border-radius:8px;">
